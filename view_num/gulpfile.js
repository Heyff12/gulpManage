...
var file_road = {
    ...
    cssDst_src_mid: '../bin/static/new/css/**/*.css',
...
gulp.task('css_num_build', function() { //替换版本号、增加特定头部代码、替换静态资源路径
...
//测试--生成common/plug版本号替换
gulp.task('js_num_build', function() { //替换版本号、增加特定头部代码、替换静态资源路径
    gulp.src(['rev/**/*.json', file_road.reqconjs_src_mid])
        .pipe(revCollector({ replaceReved: true })) //替换版本号
        .pipe(replace('../../../../static_dest/static', '/qudao/v1/static'))
        .pipe(replace('../../../static_dest/static', '/qudao/v1/static'))
...
// 图片处理------------------------------------------------------------------------------------------------------------------------------------------
gulp.task('images_dev', function() { //压缩，实时刷新--增加cache无法生成之前没有生成的文件
...
gulp.task('del_dev_npm', function(cb) {
...
//最终生成并提交代码过程中  gulp del_dev_npm && gulp static_dev && gulp html_dev && gulp del_build && gulp static_build（先删除，再生成，在替换版本号，再删除bin和template,再转移文件）
//初始化静态资源
...
gulp.task('static_build_test', function(done) {
    runSequence(
        ['images_build', 'css_build'], ['jsnum_build'], ['js_num_build'], ['html_build'], ['js_num_build'],['source_build'],
        done);
});
