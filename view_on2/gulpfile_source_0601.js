...
var gulp = require('gulp'), //基础库
    ...
    connect = require('gulp-connect'), //搭建服务器并自动更新更改--本文件没用使用，而是用的browser-sync（暂时没用）
    livereload = require('gulp-livereload'), //livereload,可以合上面配合使用（暂时没用）
    buffer = require('vinyl-buffer'),//没有达到配合gulp-sourcemaps使用的效果，没有实用（暂时没用）
    gutil = require('gulp-util'),//没有达到配合gulp-sourcemaps使用的效果，没有实用（暂时没用）
    webpack = require('webpack-stream'),//没有达到配合gulp-sourcemaps使用的效果，没有实用（暂时没用）
    named = require('vinyl-named'),//没有达到配合gulp-sourcemaps使用的效果，没有实用（暂时没用）
    through = require('through2'),//没有达到配合gulp-sourcemaps使用的效果，没有实用（暂时没用）
    sourcemaps = require('gulp-sourcemaps'); //soucemap--生成，，sourcemaps.write('../maps/js')——>sourcemaps.write(),变成行内sourcemaps，解决了部分map不生成的问题
var file_road = {
    cssSrc: './src/less/**/*.less', //开发过程——带编译less源文件——less编译，px rem转化，压缩，生成sourcemaps,,实时刷新
    cssDst: './static_dest/static/new/css', //开发过程——生成地址
    cssSrc_mid: './static_dest/static/new/css/**/*.css', //build中带转移的css源文件（待生成版本号）
...
// var ee=new Emitter.EventEmitter();
