...
var file_road = {
    ...
    cssSrc_mid: './static_dest/static/new/css/**/*.css', //build中css源文件（压缩，生成sourcemaps,）
    cssDst_map: './dist/css', //build中被压缩和生成sourcemaps的css源文件
    cssDst_src_map: './dist/css/**/*.css', //build中css源文件（待生成版本号）
    ...
    jsNum_map: './dist/js', //build——每个页面的独立js 源文件——生成版本号
    jsNum_src_map: ['./dist/js/**/*.js', '!./dist/js/common/*.js', '!./dist/js/plug/**/*.js', '!./dist/js/require.2.1.11.min.js', '!./dist/js/require-config.js', '!./dist/js/require-config2.js'],
    //build——每个页面的独立js 源文件——生成版本号
    ...
    w_cleanall_dev: ['./static_dest/static/', './static_dest/template/', './rev/'], //开发过程——删除的文件范围
    w_cleanall_dev_npm: ['./static_dest/static/new/{css,js}/**/*', './static_dest/template/**/*', './rev/', './dist/'],
    //执行npm run build 时——删除文件范围，只是不删除开发过程中生成的图片，（由于图片生成时间慢，并且不能够一次性生成，用以防止某些图片没有在压缩的过程中生成）
    w_cleanall_build: ['../bin/static/new/**/*', '../bin/template/**/*.html', '!../bin/template/b/activity_t.html', '!../bin/template/b/activity_detail__temp.html'],
    //build——删除文件范围    

    //sourcemaps
    source_src: './dist/maps/**/*.map', //build--sourcemaps源文件
...
var url_online = '/prepaid/v1/static';//线上静态资源路径
...
gulp.task('css_num_build', function() { //替换图片版本号
    gulp.src(['rev/img/*.json', file_road.cssDst_src_mid])
...
gulp.task('reqconjs_build', function() { //替换静态路径
    gulp.src(file_road.reqconjs_src_mid)
        .pipe(replace('../../../static_dest/static', url_online))
        .pipe(replace('../../static_dest/static', url_online))
...
gulp.task('jsnum_build_map', function() { //生成sourcemaps，压缩，
    gulp.src(file_road.jsNum_src_mid)
        ...
        .pipe(sourcemaps.write('../maps/js')) //soucemap生成
        .pipe(gulp.dest(file_road.jsNum_map)) //最终目录
});
gulp.task('jsnum_build', function() { //生成版本号
    gulp.src(file_road.jsNum_src_map)
        .pipe(rev()) //增加版本号
        .pipe(gulp.dest(file_road.jsDst_end)) //最终目录
...
gulp.task('html_build', function() { //替换版本号、增加特定头部代码、替换静态资源路径
    gulp.src(['rev/**/*.json', file_road.htmlSrc_mid])
        .pipe(revCollector({ replaceReved: true })) //替换版本号
...
//最终生成并提交代码过程中  gulp del_dev_npm && gulp static_dev && gulp del_build && gulp build_map && gulp static_build && gulp build_rev_replace（
...
gulp.task('build_map', function(done) {
    runSequence(
        ['css_build_map', 'jsnum_build_map'],
        done);
});
...
gulp.task('build_rev_replace', function(done) {
    runSequence(
        ['html_build', 'css_num_build'],
        done);
});
